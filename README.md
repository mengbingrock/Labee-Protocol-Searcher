...
<!-- HEALTH:BEGIN -->
_Measured automatically by [`scripts/health-check.mjs`](scripts/health-check.mjs), re-run daily by [the health workflow](.github/workflows/health.yml). Last run: **2026-09-17T10:04Z** · probe query `PCR purification` (`EcoRI` for REBASE)._
...
❌ **2 backends not answering:** `europepmc`, `semanticscholar`. The chains fall through, so search still works as long as one provider per chain is up.
...
| `europepmc` | journal | ❌ europepmc: Europe PMC HTTP 503 |
...
| --- | --- | --- | --- |
| `star-protocols` | ✅ full | ✅ 9 | ⚠️ `error` |
| `nature-protocols` | ⚠️ partial | ✅ 9 | ⚠️ `error` |
| `jove` | ⚠️ partial | ✅ 5 | ✅ `ok` · NCBI author manuscript |
| `bio-protocol` | ✅ full | ✅ 7 | ✅ `ok` |
| `current-protocols` | ✅ full | ✅ 6 | ⚠️ `abstract-only` · Europe PMC abstract |
| `protocols-io` | ⚠️ partial | ✅ 3 | ❌ `not-fetchable` |
...
| `rebase` | ✅ full | ❌ fetch failed | — not probed |

**Per-DOI retrieval:** 9/36 returned full text in this run. Not persisted: the result depends on the network the probe ran from, so it is reported, not published as a fact.
