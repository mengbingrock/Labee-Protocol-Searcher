...
<!-- HEALTH:BEGIN -->
_Measured automatically by [`scripts/health-check.mjs`](scripts/health-check.mjs), re-run daily by [the health workflow](.github/workflows/health.yml). Last run: **2026-09-23T10:10Z** · probe query `PCR purification` (`EcoRI` for REBASE)._
...
**Per-DOI retrieval:** 4/7 returned full text in this run. Not persisted: the result depends on the network the probe ran from, so it is reported, not published as a fact.
